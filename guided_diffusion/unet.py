...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        # print("input x shape = ",x.shape)     # [4,4,256,256]
        # print("time t = ",timesteps)          # timestep t->  [126,582,411,256]
        hs = []
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        # print("emb shape = ",emb.shape)      # [4,256]
        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y)
        # print("emb shape = ", emb.shape)     # [4,256]
        h = x.type(self.dtype)
        # print("h shape = ",h.shape)          # [4,4,256,256]
        c = h[:,:-1,...]                       # [4,3,256,256]
        # print("c shape = ",c.shape)          # [4,3,256,256]
        hlist= []
        # print("emb shaep = ",emb.shape)  [5,256]
        for ind, module in enumerate(self.input_blocks):
            if len(emb.size()) > 2:
                emb = emb.squeeze()
            h = module(h, emb)
            # print("input h shape = ",h.shape)  # [4,64,256,256]->[128]->[64]->[4,128,64,64]->[32]->[16]->[4,256,16,16]->8
            hs.append(h)

        uemb, cal = self.highway_forward(c, [hs[3],hs[6],hs[9],hs[12]])
        # print("uemb shape = ",uemb.shape)       # uemb[4,512,8,8]   cal=[4,1,256,256]
        h = h + uemb
        ...
        h = h.type(x.dtype)
        out = self.out(h)
        # print("out shape ={}".format(out.shape))      # [4,2,256,256]
        return out, cal
...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x, hs):
        # print("general unet x ={}".format(x.shape))  # [4,3,256,256]
        # for idx in hs:                               # [64,128] [64,64] [128,32]  [128,16]
        #     print("hs shape = ",idx.shape)
        skips = []
        seg_outputs = []
        for d in range(len(self.conv_blocks_context) - 1):
            ...
            if not self.convolutional_pooling:
                x = self.td[d](x)
            # print("x shape = ",x.shape)   # [32,128]->[64,64]->[128,32]->[256,16]->[480,8]
            if hs:
                h = hs.pop(0)
                ddims = h.size(1)
        ...
        emb = conv_nd(2, x.size(1), 256, 1).to(device = x.device)(x)
        ...
        if self._deep_supervision and self.do_ds:
            ...
            # [4,256,8,8]      [4,1,256,256]
            # print("emb shape ={} | seg_outputs shape ={}".format(emb.shape,seg_outputs[-1].shape))
