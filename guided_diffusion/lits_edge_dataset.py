...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print("npimage shape = {} | npmask shape = {}".format(npimage.shape,npmask.shape))
        npimage = ndi.zoom(npimage, (256 / npimage.shape[0], 256 / npimage.shape[1], 1), order=2)
        npmask = ndi.zoom(npmask, (256 / npmask.shape[0], 256 / npmask.shape[1]), order=0)
        ...
        nplabel = np.empty((256,256,1))
        nplabel[:, :, 0] = liver_label
        nplabel = nplabel.transpose((2, 0, 1))

        npdismap = np.empty((256, 256, 1))
        npdismap[:, :, 0] = dismap
        npdismap = npdismap.transpose((2, 0, 1))

        nplabel = nplabel.astype("float32")
        npimage = npimage.astype("float32")
        npdismap = npdismap.astype('float32')
        if not self.val:
            return npimage,nplabel,npdismap
        else:
            return npimage,nplabel,npdismap,filename