...
def standardize(img):
    ...
    img = (img - mean) / std
    return img
def dice_coef_test(output,target):
    smooth = 1e-5
    num = output.shape[0]
    ...
    inter_1 = (input_1*target_1)
    # inter_2 = (input_2*target_2)
    liver_dice = (2.*inter_1.sum()+smooth)/(input_1.sum()+target_1.sum()+smooth)
    # tumor_dice = (2.*inter_2.sum()+smooth)/(input_2.sum()+target_2.sum()+smooth)
    return liver_dice,0.0# tumor_dice
def segloss(input, target):
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        """
        # print("x shape = ",x.shape)     # [1,4,256,256]
        ...
        cal = 0
        assert t.shape == (B,)
        cal,model_output = model(x, self._scale_timesteps(t), **model_kwargs)
        # if isinstance(model_output, tuple):
        #     cal,model_output = model_output
        x=x[:,-1:,...]  #loss is only calculated on the last channel, not on the input brain MR image
        # print("self.model_val_type = ",self.model_var_type)   # Fixed_LARGE
        ...
        def process_xstart(x):
            ...
            if clip_denoised:
                return x.clamp(-1, 1)
            return x
        # print("self.model_mean_type = ",self.model_mean_type)    ModelMeanType.EPSILON
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps):
        # print("x_t shape = {} | eps shape ={}".format(x_t.shape,eps.shape))
    ...
    def p_sample_loop_known(
        ...
        img = img.to(device)
        slice_dice_res = {'liver_dice': 0.0, 'tumor_dice': 0.0, 'liver_save_numpy': None, 'tumor_save_numpy': None}
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final = sample
            # print("final[cal] shaep = ",final["cal"].shape)   # [1,2,256,256]
            # print("final[sample] shaep = ",final["sample"].shape) # [1,1,256,256]
            # dice compute
            liver_dice, tumor_dice = dice_coef_test(final['cal'], seg_gt)
            # get best
            if liver_dice > slice_dice_res['liver_dice']:
                slice_dice_res['liver_dice'] = liver_dice
                slice_dice_res['liver_save_numpy'] = torch.sigmoid(final['cal']).data.cpu().numpy()[0][0]
            """if tumor_dice > slice_dice_res['tumor_dice']:
                slice_dice_res['tumor_dice'] = tumor_dice
                slice_dice_res['tumor_save_numpy'] = torch.sigmoid(final['cal']).data.cpu().numpy()[0][1]"""
        # dismap update seg
        print("seg max = {} | seg min ={}".format(final['cal'].max(), final['cal'].min()))
        print("edge max = {} | seg min ={}".format(final['sample'].max(), final['sample'].min()))
        ...
        return final["sample"], x_noisy, img, final["cal"], cal_out, slice_dice_res
    def dice_compute(self,gt,sample,smooth=1e-5):
        """
            gt:[256,256] ---numpy
            sample:[256,256] ---numpy

        """
        y_true_f = gt.flatten()
        y_pred_f = sample.flatten()
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
           ...
           # print("******************** indecs = ",indices)
           for ipx in range(len(indices)):
                i = indices[ipx]
                t = th.tensor([i] * shape[0], device=device)
                if i%100==0:
                    print('sampling step', i)
                    # 900 -> 800 -> .....100....->0
                    viz.image(visualize(img.cpu()[0, -1,...]), opts=dict(title=slice_ID,caption="sample"+ str(i) ))
                # save predict dismap
                """if i==0:
                    xximg = img.cpu()[0, -1,...].numpy()*255
                    xximg = Image.fromarray(xximg)
                    xximg.convert('L').save("/home/luosy/EHDiffsegLIver/img_out/dismap/"+slice_ID+".jpg")"""
                with th.no_grad():
                    ...
                    # print("img shape = ",img.shape)  [1,4,256,256]
                    out = self.p_sample(
                        model,
                        img.float(),
                        t,
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t,cond, model_kwargs=None, noise=None):
        """
        ...
        """
        # print("x_start shape = {} | cond shape = {} | ".format(x_start.shape,cond.shape)) # [5,4,256,256] --- t[89,79,299,531,735] ---
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
        ...
        res_t = self.q_sample(dismap, t, noise=noise)     # add noise to the segmentation channel
        # print("res_t shaep = ",res_t.shape)          # [5,1,256,256]  add noisy label
        x_t=x_start.float()
        x_t[:, -1:, ...]=res_t.float()                  # replace label with noisy label
        terms = {}

        # print("self.loss_type = ",self.loss_type)     MSE
        # print("model_val_type = ",self.model_var_type)  ModelVarType.FIXED_LARGE
        # print("model_mean_type = ",self.model_mean_type)  # ModelMeanType.EPSILON
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:

            cal,model_output = model(x_t,self._scale_timesteps(t), **model_kwargs)
            if self.model_var_type in [
                ...
                model_output, model_var_values = th.split(model_output, C, dim=1)
                print("model_output shape ={} | model_var_values shape ={}".format(model_output.shape,model_var_values.shape))
            ...
            target = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=dismap, x_t=res_t, t=t
                )[0],
                ModelMeanType.START_X: dismap,
            ...
            # print("target shaep = {} | model_out shape ={}".format(target.shape,model_output.shape))
            # print("cal out shape = {} | seg label shaep ={}".format(cal.shape,cond.shape))
            # [5,1,256,256] --- [5,2,256,256]
            terms["mse_diff"] = mean_flat((target - model_output) ** 2 )      # target->noisy
            terms["loss_cal"] = segloss(cal,cond)# mean_flat((res - cal) ** 2)                   # res->
