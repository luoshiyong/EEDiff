...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        # print("loader len = ",len(self.dataloader))
        # 0 0 0 0
        while (
            ...
            try:
                    ...
                    batch,cond,dismap = next(data_iter)
                    # [5,3,256,256] --- [5,1,256,256]
                    # print("batch shape ={} | cond shape = {}".format(batch.shape,cond.shape))
            ...
            self.run_step(batch, cond,dismap)
            ...
            if self.step % self.log_interval == 0:
                logger.dumpkvs()
            # print("self.step = {} | self.interval = {}".format(self.step,self.save_interval))
    ...
    def run_step(self, batch, cond,dismap):
        batch=th.cat((batch, dismap), dim=1)   # [5,4,256,256]

        cond1={}
        sample = self.forward_backward(batch, cond1, cond)
    ...
    def forward_backward(self, batch, cond1,cond):

        self.mp_trainer.zero_grad()
        for i in range(0, batch.shape[0], self.microbatch):      # (0,5,5)
    ...
    def save(self):
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                ...
                # print("--------> rate = ",rate)
                if not rate:   # rate = 0 -> filename = savedmodel000000.pt
                    filename = f"savedmodel{(self.step+self.resume_step):06d}.pt"
                else:          # rate = 0.9999 filename =  emasavedmodel_0.9999_000000.pt
                    filename = f"emasavedmodel_{rate}_{(self.step+self.resume_step):06d}.pt"
                # print("_________> filename = ",filename)
